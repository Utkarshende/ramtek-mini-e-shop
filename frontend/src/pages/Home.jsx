    ...
    const fetchProducts = async () => {
      ...
      try {
        const { data } = await API.get(
          `/products/all?category=${selectedCategory}&search=${searchTerm}`
        );
        ...
        setLoading(false);
        setIsInitialLoad(false); // only false after first fetch
      }
    };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-slate-500 font-medium">
            Find the best deals in your local bazar.
          </p>
        </div>
          ...
          <div className="flex flex-col justify-center items-center h-96 text-center space-y-6">
            ...
            <div className="max-w-md bg-slate-900 border border-blue-500/30 p-6 rounded-3xl shadow-2xl">
              <p className="text-slate-300 text-sm leading-relaxed">
                <span className="text-blue-500 font-bold block mb-2">
                  🚀 Waking up the server...
                </span>
                Note: This may take 1-2 minutes on first visit.
                Our backend on Render sleeps after inactivity.
              </p>
            </div>
          </div>
        ) : (
          <>
            {/* Normal Loading Spinner (for search/filter only) */}
            {loading && (
              <div className="flex justify-center items-center py-20">
                <div className="text-blue-500 animate-spin text-4xl">⚙️</div>
              </div>
            )}
